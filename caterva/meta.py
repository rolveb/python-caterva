...
class VLMeta(Mapping):
    """
    Class providing access to user vlmeta on a :py:class:`NDArray`.
    It will be available via the `.vlmeta` property of an array.
    """
    def get(self, key, default=None):
        """Return the value for `key` if `key` is in the dictionary, else `default`.
        If `default` is not given, it defaults to ``None``."""
    ...
    def __contains__(self, key):
        """Check if the `key` metalayer exists or not."""
        return ext.vlometa__contains__(self.arr, key)
    ...
    def __setitem__(self, key, value):
        ...
        """
        return ext.vlmeta__setitem__(self.arr, key, value)
    ...
    def __getitem__(self, item):
        ...
        bytes
            ...
            format).
        """
        return ext.vlmeta__getitem__(self.arr, item)

    def keys(self):
        """Return the metalayers keys."""
        return ext.vlmeta_keys(self.arr)
    ...
    def __len__(self):
        """Return the number of metalayers."""
        return ext.vlmeta__len__(self.arr)
